"""POST /ask — the general ReAct tool-calling endpoint.

Flow:
  1. If the question matches a fast intent (e.g. "fill this form"), skip
     the ReAct loop entirely — dispatch straight to the target tool, build
     the response contract in Python, log to memory, return.
  2. Otherwise recall relevant past interactions from memory, seed a
     system+user message pair, and run up to MAX_ITERS rounds of the
     Ollama /api/chat tool-calling loop. Each tool call goes through the
     registry; side-effecting / requires_confirmation tools halt the loop
     with a draft the panel renders.
"""
